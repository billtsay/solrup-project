...
package com.solrup.evtgen.engine.scxml;
...
import org.finra.datagenerator.engine.scxml.tags.*;
...
public class SCXMLEngine extends SCXMLExecutor implements Engine {
    ...
    private Map<String, String> fillInitialVariables() {
        ...
        for (EnterableState target : targets) {
            for (OnEntry entry: target.getOnEntries()){
    ...
    public List<PossibleState> bfs(int min) throws ModelException {
        ...
        bootStrap.add(new PossibleState(model.getTargets().get(model.getInitial()), fillInitialVariables()));
    ...
    public void setModelByInputFileStream(InputStream inputFileStream) {
        try {
            this.model = SCXMLReader.read(inputFileStream, new SCXMLReader.Configuration(null, null,customActionsFromTagExtensions()));
    ...
    public void setModelByText(String model) {
        try {
            ...
            this.model = SCXMLReader.read(is, new SCXMLReader.Configuration(null, null, customActionsFromTagExtensions()));
            this.setStateMachine(this.model);
        } catch (IOException | XMLStreamException | ModelException e) {
            e.printStackTrace();
        }
